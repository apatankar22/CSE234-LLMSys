...
def get_info(comm, rank: int, mp_size: int, dp_size: int, fc_layer: str, in_dim: int, out_dim: int,):
    """
    ...
    if fc_layer != "fc_o":
        pi_dim = in_dim
        po_dim = out_dim // mp_size
        
    else:
        pi_dim = in_dim // mp_size
        po_dim = out_dim
    
    return mp_idx, dp_idx, mp_comm, dp_comm, pi_dim, po_dim
...
#    return collected_x
    ...
    return np.concatenate(nodes_acc, axis=2)
...
def naive_collect_forward_output(
    ...
    collected_out = np.empty((batch_size, sequence_length, part_out_dimension * mp_size), dtype=out.dtype)
    receive_buffer = np.empty([mp_size, batch_size, sequence_length, part_out_dimension], dtype=out.dtype)
    mp_comm.Allgather(out.copy(), receive_buffer)
...
def naive_collect_backward_output(
    ...
    temp = out_dim // mp_size  
    init_idx = mp_group_idx * temp

    return output_grad[:, :, init_idx:init_idx + temp].copy()
...
def naive_collect_backward_x(
    ...
    glob_x = np.empty_like(grad_x)
    mp_comm.Allreduce(sendbuf=grad_x, recvbuf=glob_x, op=MPI.SUM)

    collected_grad_x = np.empty((batch_size, seq_length, in_dim // mp_size), dtype=grad_x.dtype)
    scatter_buffer = np.split(glob_x, mp_size, axis=2)
