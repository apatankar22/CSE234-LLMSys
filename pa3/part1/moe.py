...
class ShardedLinear:
    ...
    def __call__(self, x):
        ...
        temp = self.output_offset
        result[:, temp: temp + self.local_out_features] = x @ self.weight + self.bias
        return result
...
class MoE_TP:
    ...
    def forward(self, x):
        ...
        expert_dims = self.output_dim //self.world_size
        ...
        for i in range(self.topk):
            for j in range(self.topk):
                temp = x[j: j+1]
                temp_output = self.experts[indices[j, i]](temp)
                outputs[j] += temp_output[0] * gates[j, i]
                
        sol = mpi.allreduce(outputs)
        return sol
...
class MoE_EP:
    ...
    def forward(self, x):
        ...
        for i in range(batch_size):
            for j in range(self.topk):
                if indices[i, j] == self.rank:
                    temp = self.expert(x[i: i+1])
                    outputs[i] += (gates[i, j] * temp[0])
